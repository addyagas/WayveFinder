...
app.get('/login', async (req, res) => {
    try {
        ...
        const scope = 'user-library-read user-top-read playlist-read-private app-remote-control streaming';
...
app.post('/get_info', async (req, res) => {
    ...
    try {
        ...
        console.log('Access token expiry:', new Date(req.session.expiresAt).toISOString());
        ...
        console.log('Full song data response:', {
        ...
        if (response.status !== 200) {
            return res.status(response.status).send({ error: `Spotify API error: ${response.statusText}` });
        }
        ...
        // Log full audio features response for debugging
        console.log('Full audio features response:', {
            status: audioFeatures.status,
            statusText: audioFeatures.statusText,
            headers: audioFeatures.headers,
            data: audioFeatures.data,
        });

        if (audioFeatures.status !== 200) {
            return res.status(audioFeatures.status).send({ error: `Spotify API error: ${audioFeatures.statusText}` });
        }
        ...
        console.error('Error fetching song or audio features:', error.response ? error.response.data : error.message); // Detailed error logging
        res.status(500).send({ error: 'Error fetching song or audio features' });
    }
});
